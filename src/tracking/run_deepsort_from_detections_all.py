...
def main():
    ...
    parser.add_argument("--max-cosine-distance", default="0.2")
    parser.add_argument("--nn-budget", default="100")
    args = parser.parse_args()

    os.makedirs("outputs/detections_npy", exist_ok=True)
    os.makedirs(f"outputs/tracks/{args.tracker_name}", exist_ok=True)

    for seq in SEQUENCES:
        print("=" * 80)
        print(f"Running DeepSORT for {seq}")

        det_txt = f"{args.det_dir}/{seq}.txt"
        det_npy = f"outputs/detections_npy/{args.tracker_name}_{seq}.npy"
